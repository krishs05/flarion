use crate::config::{BackendType, Cli, ConfigError, FlarionConfig, ModelConfig};
use crate::engine::backend::InferenceBackend;
use crate::engine::llama::LlamaBackend;
use crate::engine::registry::BackendRegistry;
use crate::{engine, metrics, routing, server};
use std::sync::Arc;
...
pub async fn run(cli: Cli) -> anyhow::Result<()> {
    ...
    let scheduler = crate::engine::scheduling::Scheduler::new(budgets.clone());
    for (gpu_id, &budget_mb) in budgets.iter().enumerate() {
        crate::metrics::set_vram_budget_on_gpu(gpu_id as u32, budget_mb);
    }
    ...
    let evictor: Arc<dyn crate::engine::backend::Evictor> = registry.clone();
...
async fn load_backend(
    cfg: &ModelConfig,
    scheduler: std::sync::Arc<crate::engine::scheduling::Scheduler>,
    ...
    match cfg.backend {
        BackendType::Local => {
            ...
            let estimated_mb = crate::engine::scheduling::estimate_vram_mb(path, cfg.vram_mb)
        ...
        BackendType::Hf => {
            #[cfg(feature = "hf_cuda")]
            {
                let backend = crate::engine::hf::HfBackend::new(cfg)
...
mod tests {
    use super::*;
    use crate::config::{BackendType, ModelConfig, ServerConfig};
